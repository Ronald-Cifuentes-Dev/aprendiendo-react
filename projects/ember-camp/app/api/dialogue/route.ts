...
type RequestBody = { missionId?: string; message?: string; history?: DialogueMessage[]; goalCompleted?: boolean };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (typeof parsed.reply !== 'string' || typeof parsed.understood !== 'boolean' || typeof parsed.goalCompleted !== 'boolean') throw new Error('Invalid dialogue schema');
    return NextResponse.json({ ...parsed, goalCompleted: goalCompleted || parsed.goalCompleted, source: 'ai' } satisfies DialogueResult);
  } catch (error) {
